...
void drawBar() {
 drawRect(0, 120, 240, 3, ((17) | (30)<<5 | (25)<<10));
}
...
void updateCannon() {
 ...
 if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<5)))
  ...
  && cannon.col + cannon.width - 1 < 240 - cannon.cdel) {

  cannon.col += cannon.cdel;

 }

 if ((!(~(oldButtons)&((1<<1))) && (~buttons & ((1<<1)))) && cannon.bulletTimer >= 20) {
...
void drawCannon() {
 ...
 drawRect(cannon.oldCol, cannon.oldRow, cannon.width, cannon.height, 0);
...
void updateBall(BALL* b) {
 ...
 if (b->active) {
  ...
  for (int j=0; j<5; j++) {
   for (int i=0; i<5; i++) {
    if(bullets[j].active && balls[i].active) {
     if(collision(bullets[j].col, bullets[j].row, bullets[j].width, bullets[j].height, balls[i].col, balls[i].row, balls[i].width, balls[i].height)) {
      bullets[j].active = 0;
      balls[i].active = 0;
      ballsRemaining -=1;
     }
