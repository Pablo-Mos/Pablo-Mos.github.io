...
# 1 "collisionMap.h" 1
# 21 "collisionMap.h"
extern const unsigned short collisionMapBitmap[19200];
...
void initPlayer()
{
    ...
    player.row = 160 - player.height - 16;
    player.aniCounter = 0;
    player.curFrame = 0;
    player.aniState = 1;
    player.numFrames = 3;
...
# 206 "game.c"
    ...
    for (int i = 0; i < 1; i++)
    {
        if (gun[i].active == 1)
        {
            shadowOAM[3 + i].attr0 = gun[i].row | (0 << 13) | (0 << 14);
            shadowOAM[3 + i].attr1 = gun[i].col | (2 << 14);
            shadowOAM[3 + i].attr2 = ((0) << 12) | ((4 * gun[i].curFrame)*32 + (24));
        }
        else
        {
...
void updateEnemy()
{
    ...
    if (enemy.active)
    {
        shadowOAM[2].attr0 = enemy.row | (0 << 13) | (0 << 14);
        shadowOAM[2].attr1 = enemy.col | (2 << 14);
        shadowOAM[2].attr2 = ((0) << 12) | ((enemy.curFrame * 4)*32 + (16));
...
void updatePlayer()
{
    ...
    if (hasGun && (!(~(oldButtons) & ((1 << 0))) && (~buttons & ((1 << 0)))) && player.bulletTimer >= 10)
    {
        fireBullet();
    ...
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1 << 5))) && (player.col > 0) && collisionCheck(collisionMap, 240, player.col, player.row, player.width, player.height, -player.cdel, 0))
    {
        player.aniState = SPRITELEFT;
        player.col -= player.cdel;
    }
    else if ((~((*(volatile unsigned short *)0x04000130)) & ((1 << 4))) && (player.col + player.width - 1 < 240) && collisionCheck(collisionMap, 240, player.col, player.row, player.width, player.height, player.cdel, 0))
    {
    ...
    if (!jumped && collisionMap[((player.row + player.height) * (240) + (player.col))] != 0 && collisionMap[((player.row + player.height) * (240) + (player.col + (player.width / 2)))] != 0 && collisionMap[((player.row + player.height) * (240) + (player.col + player.width - 1))] != 0)
        player.row++;
}
...
void drawBullets()
{
    for (int i = 0; i < 5; i++)
    {
        if (bullets[i].active)
        {
            ...
            {
                shadowOAM[i + 19].attr0 = (0xFF & bullets[i].row) | (0 << 13) | (1 << 14);
                shadowOAM[i + 19].attr1 = (0x1FF & bullets[i].col) | (0 << 14);
                shadowOAM[i + 19].attr2 = ((0) << 12) | ((0)*32 + (20));
            }
            else
            {
                shadowOAM[i + 19].attr0 = (0xFF & bullets[i].row) | (0 << 13) | (2 << 14);
                shadowOAM[i + 19].attr1 = (0x1FF & bullets[i].col) | (0 << 14);
                shadowOAM[i + 19].attr2 = ((0) << 12) | ((0)*32 + (22));
            }
        }
        else
        {
            shadowOAM[i + 19].attr0 = (2 << 8);
        }
    }
}
...
void drawPlayer()
{
    ...
    if (jumped)
    {
        ...
        if (timer % 2 == 0)
        {
            ...
            if (jumpCount == maxJump || (collisionMap[((player.row - 1) * (240) + (player.col))] == 0 || collisionMap[((player.row - 1) * (240) + (player.col + (player.width / 2)))] == 0 || collisionMap[((player.row - 1) * (240) + (player.col + player.width - 1))] == 0))
            {
                jumped = 0;
                fallen = 1;
            }
    ...
    else if (collisionCheck(collisionMap, 240, player.col, player.row + 2, player.width, player.height, 0, player.rdel))
        player.row++;
    else if (fallen)
    {
        if (player.prevAniState == SPRITERIGHT || player.aniState == SPRITERIGHT)
