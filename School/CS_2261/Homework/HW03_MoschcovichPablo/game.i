...
void drawGame() {
 ...
 for (int i = 0; i < 15 -dif; i++)
  drawFood(&foods[i]);
    ...
    setPixel(rand()%240,rand()%140,((0) | (5)<<5 | (31)<<10));
    drawCircle(rand()%240,rand()%135,10,((0) | (0)<<5 | (10)<<10));
}
...
void drawPlayer() {
 drawRect(player.oldCol, player.oldRow, player.width, player.height, ((0) | (0)<<5 | (10)<<10));
 drawRect(player.col, player.row, player.width, player.height, player.color);
 drawRect(player.col+1, player.row+3, 3, 1, ((31) | (31)<<5 | (0)<<10));
 setPixel(player.col+1,player.row+1,((31) | (31)<<5 | (0)<<10));
 setPixel(player.col+3,player.row+1,((31) | (31)<<5 | (0)<<10));
  ...
  if(i==0)
   ...
   waves[i].row = 120+ rand() % (20);
      ...
      waves[i].col = 0;
       ...
       waves[i].col = 240 -waves[i].width-2;
  waves[i].oldRow = player.row;
  waves[i].oldCol = player.col;
  waves[i].cdel = -2;

  if(i%2==0)
   waves[i].color = ((0) | (25)<<5 | (31)<<10);
  else
   waves[i].color = ((0) | (19)<<5 | (31)<<10);
  waves[i].active = 1;
  waves[i].erased = 0;
 }
}
...
void drawWave(WAVE* w) {
 if(w->active) {
  drawRect(w->oldCol, w->oldRow, w->width, w->height, ((0) | (0)<<5 | (10)<<10));
...
void initFoods() {
 for (int i = 0; i < 15 -dif; i++) {
  foods[i].height = 3;
  foods[i].width = 3;
  foods[i].row = rand() % (160 -foods[i].height-1);
   foods[i].col = rand() % (240 -foods[i].width-1);
  foods[i].oldRow = player.row;
  foods[i].oldCol = player.col;
  foods[i].color = ((0) | (31)<<5 | (0)<<10);
  foods[i].active = 1;
  foods[i].erased = 0;
 }
}
  ...
  if (collision(f->col, f->row, f->width, f->height, player.col, player.row, player.width, player.height)) {
    ...
    for (int i = 0; i < 5; i++) {
        if (collision(f->col, f->row, f->width, f->height,waves[i].col, waves[i].row, waves[i].width, waves[i].height)) {
            f->row = rand() % (160 -foods[i].height-1);
       f->col = rand() % (240 -foods[i].width-1);
        }
    }
}
...
void drawFood(FOOD* f) {
    if(f->active) {
  drawRect(f->oldCol, f->oldRow, f->width, f->height, ((0) | (0)<<5 | (10)<<10));
  drawRect(f->col, f->row, f->width, f->height, f->color);
  setPixel(f->col+1, f->row+1,((0) | (20)<<5 | (0)<<10));
