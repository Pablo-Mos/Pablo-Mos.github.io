...
# 1 "ship.h" 1
# 21 "ship.h"
extern const unsigned short shipBitmap[72];
...
void initGame() {
    ...
    DMANow(3, &shipPal, ((unsigned short *)0x5000000), 256);
    for (int i = 0; i < 6; i++) {
...
void initPlayer() {
    player.col = 240 -player.width-16;
    player.row = 160/2 -10;
    player.width = 12;
    player.height = 12;
    player.cdel = 1;
...
void updatePlayer() {
    ...
    if ((!(~(oldButtons)&((1<<1))) && (~buttons & ((1<<1)))) && player.bulletTimer >= 15) {
        fireBullet();
...
void updateEnemy(ENEMY* a) {
    if (a->active) {
        ...
        if (a->col <= 1 || a->col + a->width+dim - 1 > 240 - 2 || a->col + a->width+dim - 1 < 25+dim){
            a->cdel *= -1;
        }
        if (a->row <= 1 || a->row + a->height+dim - 1 > 160) {
            a->rdel *= -1;
        }
        ...
        for (int i = 0; i < 8; i++) {
            if (bullets[i].active && collision(a->col, a->row, a->width+dim, a->height+dim,
                bullets[i].col, bullets[i].row, bullets[i].width, bullets[i].height)) {
                    ...
                    *(volatile u16*)0x0400006C = NOTE_B4 | (1<<15) | (1<<14);
            }
        }
        ...
        if (collision(a->col, a->row, a->width+dim, a->height+dim,
            player.col, player.row, player.width, player.height)) {
                gameState = -1;
                *(volatile u16*)0x04000068 = (((10) & 15) << 12) | (0 << 11) | (((7) & 7) << 8) | (2 << 6);
                *(volatile u16*)0x0400006C = NOTE_E4 | (1<<15) | (1<<14);
        }
    }
}
