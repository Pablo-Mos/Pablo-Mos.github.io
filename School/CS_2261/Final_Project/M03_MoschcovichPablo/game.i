...
void initEnemy()
{
    for (int i = 0; i < 3; i++)
    {
        ...
        case NORMAL:
            ...
            {
                enemy[i].worldCol = 512 - 60;
                enemy[i].worldRow = 256 - 37;
            }
...
void initCoins()
{
    for (int i = 0; i < 3; i++)
    {
        ...
        case SPECIAL:
            coin[i].width = 7;
            coin[i].height = 7;
            coin[i].worldCol = 512 - 60 - 60;
            coin[i].worldRow = 256 - 32;
            coin[i].active = 1;
            break;
        }
    }
...
void updateWeapon()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        {
            shadowOAM[i + 10].attr0 = (0xFF & weapon[i].worldRow - vOff) | (0 << 13) | (0 << 14);
            shadowOAM[i + 10].attr1 = (0x1FF & weapon[i].worldCol - hOff) | (2 << 14);
            if (weapon[i].type == GUN)
                shadowOAM[i + 10].attr2 = ((0) << 12) | ((0)*32 + (8));
            else if (weapon[i].type == KNIFE)
                shadowOAM[i + 10].attr2 = ((0) << 12) | ((4)*32 + (8));
            else if (weapon[i].type == SHIELD)
                shadowOAM[i + 10].attr2 = ((0) << 12) | ((8)*32 + (8));
        }
        else
        {
...
void updateEnemy()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (enemy[i].active)
        {
            shadowOAM[i + 3 + 10].attr0 = (0xFF & enemy[i].worldRow - vOff) | (0 << 13) | (0 << 14);
            shadowOAM[i + 3 + 10].attr1 = (0x1FF & enemy[i].worldCol - hOff) | (2 << 14);
            if (enemy[i].type == NORMAL)
                shadowOAM[i + 3 + 10].attr2 = ((0) << 12) | ((enemy[i].curFrame * 4)*32 + (16));
            else if (enemy[i].type == BOSS)
                shadowOAM[i + 3 + 10].attr2 = ((0) << 12) | ((enemy[i].curFrame * 4 + 12)*32 + (16));
...
void updateCoins()
{
    for (int i = 0; i < 3; i++)
    {
        if (coin[i].active)
        {
            shadowOAM[i + 3 + 3 + 10].attr0 = (0xFF & coin[i].worldRow - vOff) | (0 << 13) | (0 << 14);
            if (coin[i].type == NORMAL)
            {
                shadowOAM[i + 3 + 3 + 10].attr1 = (0x1FF & coin[i].worldCol - hOff) | (1 << 14);
                shadowOAM[i + 3 + 3 + 10].attr2 = ((0) << 12) | ((0)*32 + (24));
            }
            else if (coin[i].type == SPECIAL)
            {
                shadowOAM[i + 3 + 3 + 10].attr1 = (0x1FF & coin[i].worldCol - hOff) | (0 << 14);
                shadowOAM[i + 3 + 3 + 10].attr2 = ((0) << 12) | ((0)*32 + (23));
            }
...
void updateAmmo()
{
    for (int i = 0; i < 5; i++)
    {
        if (ammo[i].active)
        {
            shadowOAM[i + 3 + 3 + 3 + 10].attr0 = (0xFF & ammo[i].worldRow - vOff) | (0 << 13) | (0 << 14);
            shadowOAM[i + 3 + 3 + 3 + 10].attr1 = (0x1FF & ammo[i].worldCol - hOff) | (0 << 14);
            shadowOAM[i + 3 + 3 + 3 + 10].attr2 = ((0) << 12) | ((0)*32 + (30));
            ...
            shadowOAM[i + 3 + 3 + 3 + 10].attr0 = (2 << 8);
    }
}
...
void updateBulletEnemy(ANISPRITE *b)
{
    ...
    if (b->active)
    {
        if (b->direction == 1)
        {
            if (b->worldCol + b->cdel <= 240 + hOff - 1)
            {
                b->worldCol += b->cdel;
            }
            else
...
# 698 "game.c"
    ...
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1 << 5))) && (player.worldCol > 0) && collisionCheck(collisionMap, 512, player.worldCol, player.worldRow, player.width, player.height, -player.cdel, 0))
    {
    ...
    else if ((~((*(volatile unsigned short *)0x04000130)) & ((1 << 4))) && (player.worldCol + player.width - 1 < 512) && collisionCheck(collisionMap, 512, player.worldCol, player.worldRow, player.width, player.height, player.cdel, 0))
    {
    ...
    if (!jumped && collisionMap[((player.worldRow + player.height) * (512) + (player.worldCol))] != 0 && collisionMap[((player.worldRow + player.height) * (512) + (player.worldCol + (player.width / 2)))] != 0 && collisionMap[((player.worldRow + player.height) * (512) + (player.worldCol + player.width - 1))] != 0)
        player.worldRow++;
}
...
void drawBullets()
{
    for (int i = 0; i < 5; i++)
    {
        if (bullets[i].active)
        {
            ...
            {
                shadowOAM[i + 3 + 3 + 3 + 5 + 10].attr0 = (0xFF & (bullets[i].worldRow - vOff)) | (0 << 13) | (1 << 14);
                shadowOAM[i + 3 + 3 + 3 + 5 + 10].attr1 = (0x1FF & (bullets[i].worldCol - hOff)) | (0 << 14);
                shadowOAM[i + 3 + 3 + 3 + 5 + 10].attr2 = ((0) << 12) | ((0)*32 + (20));
            }
            else
            {
                shadowOAM[i + 3 + 3 + 3 + 5 + 10].attr0 = (0xFF & (bullets[i].worldRow - vOff)) | (0 << 13) | (2 << 14);
                shadowOAM[i + 3 + 3 + 3 + 5 + 10].attr1 = (0x1FF & (bullets[i].worldCol - hOff)) | (0 << 14);
                shadowOAM[i + 3 + 3 + 3 + 5 + 10].attr2 = ((0) << 12) | ((0)*32 + (22));
            }
        }
        else
        {
            shadowOAM[i + 3 + 3 + 3 + 5 + 10].attr0 = (2 << 8);
        }
    }
}
...
void drawBulletsEnemy()
{
    for (int i = 0; i < 30; i++)
    {
        ...
        {
            shadowOAM[i + 3 + 3 + 3 + 5 + 5 + 10].attr0 = (0xFF & (enemyBullets[i].worldRow - vOff)) | (0 << 13) | (1 << 14);
            shadowOAM[i + 3 + 3 + 3 + 5 + 5 + 10].attr1 = (0x1FF & (enemyBullets[i].worldCol - hOff)) | (0 << 14);
            shadowOAM[i + 3 + 3 + 3 + 5 + 5 + 10].attr2 = ((0) << 12) | ((0)*32 + (20));
        }
        else
        {
            shadowOAM[i + 3 + 3 + 3 + 5 + 5 + 10].attr0 = (2 << 8);
        }
    }
}
...
void drawPlayer()
{
    shadowOAM[0].attr0 = (0xFF & player.worldRow - vOff) | (0 << 13) | (0 << 14);
    ...
    if (jumped)
    {
        ...
        if (timer % 2 == 0)
        {
            ...
            if (jumpCount == maxJump || (collisionMap[((player.worldRow - 1) * (512) + (player.worldCol))] == 0 || collisionMap[((player.worldRow - 1) * (512) + (player.worldCol + (player.width / 2)))] == 0 || collisionMap[((player.worldRow - 1) * (512) + (player.worldCol + player.width - 1))] == 0))
            {
                jumped = 0;
                fallen = 1;
            }
    ...
    else if (collisionCheck(collisionMap, 512, player.worldCol, player.worldRow + 2, player.width, player.height, 0, player.rdel))
        player.worldRow++;
    else if (fallen)
    {
        if (player.prevAniState == SPRITERIGHT || player.aniState == SPRITERIGHT)
